...
class MessagesController extends Controller
{
    ...
    {
        $connection = UserConnection::where('user_id',$request['id'])->where('status',1)->pluck('connection_id')->toArray();
        $connection2 = UserConnection::where('connection_id',$request['id'])->where('status',1)->pluck('user_id')->toArray();   
        $connectionIds = array_merge($connection, $connection2);
        $favorite   = User::query()
        ...
        return Response::json([
            'favorite' => $favorite,
            'fetch' => $fetch ?? null,
            'user_avatar' => asset($fetch->logo??$fetch->profile_picture??'/images/icons/Profile.svg'),
        ]);
    }
        ...
        if (!$error->status) {
            ...
            if ($sender->id != $touser->id) {
                if($touser->user_type==3){
                    ...
                    Chatify::push('tapdigi-tradehook.'.$touser->id,'new_contact_update',[]);
                }else{
            ...
            if($touser->device_token && $touser->active_status != 1){
    ...
    public function getContacts(Request $request)
    {
        ...
        ->select('users.*',DB::raw('MAX(`apr_ch_messages`.`created_at`) max_created_at'))
