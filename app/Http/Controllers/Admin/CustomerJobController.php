...
class CustomerJobController extends Controller
{
    ...
    public function create()
    {
        ...
        if(User::ROLE['admin']){
            ...
            return view('admin.customer.job.create',['experience_range'=>$experience_range,'status'=>$status,'categories'=>$categories,'company_address'=>$company_address,'company_latitude'=>$company_latitude,'company_longitude'=>$company_longitude]);
        }
        ...
        if($subcription){
            if($subcription->job_limit > $subcription->used_job_qty){
                ...
                $experience_range = Badge::getAllBadges();
                return view('admin.customer.job.create',['experience_range'=>$experience_range,'status'=>$status,'categories'=>$categories,'agencies'=>$agencies,'company_address'=>$company_address,'company_latitude'=>$company_latitude,'company_longitude'=>$company_longitude]);
            }else{
                if($sub_user ==1){
    ...
    {        
        ...
        return redirect('jobs')->with('success', 'Job has been created successfully!');
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        if($request->ajax()){
            return response()->json(['status' => 200,'message'=>'','url'=>route("jobs.show", $model->id)]);
        }else{
            return redirect('jobs')->with('success', 'Job has been updated successfully!');
        }
    }
