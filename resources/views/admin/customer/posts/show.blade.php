...
<div class="container-fluid middle-content dashboard-content">
    <div class="page-title">
        ...
        <div class="right-title">
            <a href="{{ route('customer.posts.list') }}"><button class="primary-btn black-button">Back</button></a>
            @if($post->created_at->diffInDays(now()) <= 2)
            <a href="{{ route('customer.posts.edit', $post->id) }}"><button class="primary-btn blue-button">Edit</button></a>
            @endif
            <form action="{{ route('customer.posts.destroy', $post->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this post?')">
                @csrf
