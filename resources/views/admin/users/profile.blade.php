...
<div class="container skill-reg-form pb-5">
  ...
  {{-- Agency Details Section (user_type = 2) --}}
      ...
      <form class="regular-form" action="{{ route('updateagency') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
          <div class="mb-3">
            ...
            <textarea class="form-control" id="about_agency" name="about_agency" rows="6">{{$model->about_agency}}</textarea>
          </div>
        </div>
        <div class="row">
          <div>
  ...
  {{-- Customer Details Section (user_type = 5) --}}
        ...
        <div class="col-md-6">
          <div class="card">
            <div class="card-body">
              ...
              <p><strong>Mobile:</strong> {{ $model->mobile ?? 'N/A' }}</p>
              <p><strong>Account Type:</strong> Customer</p>
              <p><strong>Status:</strong> 
                <span class="badge {{ $model->status == 1 ? 'bg-success' : 'bg-danger' }}">
                  {{ $model->status == 1 ? 'Active' : 'Inactive' }}
                </span>
              </p>
              <p><strong>Joined:</strong> {{ $model->created_at->format('d-m-Y') }}</p>
            </div>
          </div>
        </div>
        <div class="col-md-6">
          <div class="card">
            <div class="card-body">
              ...
              @endif
              @if(isset($model->pincode) && !empty($model->pincode))
                <p><strong>Pincode:</strong> {{ $model->pincode }}</p>
              @endif
              @if(isset($model->country) && !empty($model->country))
...
{{-- Google Maps API - Only for Agency --}}
@if(Auth::user()->user_type == 2)
